/*
 * Box Platform API
 *
 * [Box Platform](https://box.dev) provides functionality to provide access to content stored within [Box](https://box.com). It provides endpoints for basic manipulation of files and folders, management of users within an enterprise, as well as more complex topics such as legal holds and retention policies.
 *
 * The version of the OpenAPI document: 2024.0
 * Contact: devrel@box.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct PostWebhooksRequest {
    #[serde(rename = "target")]
    pub target: Box<models::PostWebhooksRequestTarget>,
    /// The URL that is notified by this webhook.
    #[serde(rename = "address")]
    pub address: String,
    /// An array of event names that this webhook is to be triggered for.
    #[serde(rename = "triggers")]
    pub triggers: Vec<Triggers>,
}

impl PostWebhooksRequest {
    pub fn new(target: models::PostWebhooksRequestTarget, address: String, triggers: Vec<Triggers>) -> PostWebhooksRequest {
        PostWebhooksRequest {
            target: Box::new(target),
            address,
            triggers,
        }
    }
}
/// An array of event names that this webhook is to be triggered for.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Triggers {
    #[serde(rename = "FILE.UPLOADED")]
    FileUploaded,
    #[serde(rename = "FILE.PREVIEWED")]
    FilePreviewed,
    #[serde(rename = "FILE.DOWNLOADED")]
    FileDownloaded,
    #[serde(rename = "FILE.TRASHED")]
    FileTrashed,
    #[serde(rename = "FILE.DELETED")]
    FileDeleted,
    #[serde(rename = "FILE.RESTORED")]
    FileRestored,
    #[serde(rename = "FILE.COPIED")]
    FileCopied,
    #[serde(rename = "FILE.MOVED")]
    FileMoved,
    #[serde(rename = "FILE.LOCKED")]
    FileLocked,
    #[serde(rename = "FILE.UNLOCKED")]
    FileUnlocked,
    #[serde(rename = "FILE.RENAMED")]
    FileRenamed,
    #[serde(rename = "COMMENT.CREATED")]
    CommentCreated,
    #[serde(rename = "COMMENT.UPDATED")]
    CommentUpdated,
    #[serde(rename = "COMMENT.DELETED")]
    CommentDeleted,
    #[serde(rename = "TASK_ASSIGNMENT.CREATED")]
    TaskAssignmentCreated,
    #[serde(rename = "TASK_ASSIGNMENT.UPDATED")]
    TaskAssignmentUpdated,
    #[serde(rename = "METADATA_INSTANCE.CREATED")]
    MetadataInstanceCreated,
    #[serde(rename = "METADATA_INSTANCE.UPDATED")]
    MetadataInstanceUpdated,
    #[serde(rename = "METADATA_INSTANCE.DELETED")]
    MetadataInstanceDeleted,
    #[serde(rename = "FOLDER.CREATED")]
    FolderCreated,
    #[serde(rename = "FOLDER.RENAMED")]
    FolderRenamed,
    #[serde(rename = "FOLDER.DOWNLOADED")]
    FolderDownloaded,
    #[serde(rename = "FOLDER.RESTORED")]
    FolderRestored,
    #[serde(rename = "FOLDER.DELETED")]
    FolderDeleted,
    #[serde(rename = "FOLDER.COPIED")]
    FolderCopied,
    #[serde(rename = "FOLDER.MOVED")]
    FolderMoved,
    #[serde(rename = "FOLDER.TRASHED")]
    FolderTrashed,
    #[serde(rename = "WEBHOOK.DELETED")]
    WebhookDeleted,
    #[serde(rename = "COLLABORATION.CREATED")]
    CollaborationCreated,
    #[serde(rename = "COLLABORATION.ACCEPTED")]
    CollaborationAccepted,
    #[serde(rename = "COLLABORATION.REJECTED")]
    CollaborationRejected,
    #[serde(rename = "COLLABORATION.REMOVED")]
    CollaborationRemoved,
    #[serde(rename = "COLLABORATION.UPDATED")]
    CollaborationUpdated,
    #[serde(rename = "SHARED_LINK.DELETED")]
    SharedLinkDeleted,
    #[serde(rename = "SHARED_LINK.CREATED")]
    SharedLinkCreated,
    #[serde(rename = "SHARED_LINK.UPDATED")]
    SharedLinkUpdated,
    #[serde(rename = "SIGN_REQUEST.COMPLETED")]
    SignRequestCompleted,
    #[serde(rename = "SIGN_REQUEST.DECLINED")]
    SignRequestDeclined,
    #[serde(rename = "SIGN_REQUEST.EXPIRED")]
    SignRequestExpired,
    #[serde(rename = "SIGN_REQUEST.SIGNER_EMAIL_BOUNCED")]
    SignRequestSignerEmailBounced,
    #[serde(rename = "SIGN_REQUEST.SIGN_SIGNER_SIGNED")]
    SignRequestSignSignerSigned,
    #[serde(rename = "SIGN_REQUEST.SIGN_DOCUMENT_CREATED")]
    SignRequestSignDocumentCreated,
    #[serde(rename = "SIGN_REQUEST.SIGN_ERROR_FINALIZING")]
    SignRequestSignErrorFinalizing,
}

impl Default for Triggers {
    fn default() -> Triggers {
        Self::FileUploaded
    }
}

