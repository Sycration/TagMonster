...
**password** | Option<**String**> | The password required to access the shared link. Set the password to `null` to remove it. Passwords must now be at least eight characters long and include a number, upper case letter, or a non-numeric or non-alphabetic character. A password can only be set when `access` is set to `open`. | [optional]
...
**permissions** | Option<[**models::PutWebLinksIdUpdateSharedLinkRequestSharedLinkPermissions**](put_web_links_id_update_shared_link_request_shared_link_permissions.md)> |  | [optional]
